...
本缓存机制为 **`Axios`** 请求提供了一个可扩展的客户端缓存层，包含内存缓存与持久化缓存。当前版本未启用服务端缓存（此前的 `ServerCache` 方案已移除，以避免 `next/cache` 被引入客户端环境）。以下为客户端缓存流程说明。
